...
感觉有一个可能的改进方案：不是一有新增内容就提醒一下记忆 agent，而是新增内容每达到worker上下文的1%，就发送一次提醒一下？也许没有必要按上下文的百分之几，而是可以按条数？

记忆agent的system instruction:
```
你处于一个叫project-x的agent系统中，有一个agent(worker)和用户交互并完成用户交付的任务，你将会持续收到worker的最新上下文片段，你的任务是给worker管理记忆（~/.project-x/memories/summaries/main.md），系统会自动给worker加载这个文档
...
worker所有的完整上下文都存储在 ~/.project-x/memories/originals/ 的jsonl文件中，你可以用 grep 来搜索，你不能修改这其中的任何jsonl文件。
...
```
你处于一个叫 project-x 的 agent系统中，用户通过网页来和你交互。
...
```
这个main memory 从 ~/.project-x/memories/summaries/main.md 中读取

## 不采用的其他方案 & todo

### 不用专门搞一个agent，用llm调用即可，把worker上下文发过去让llm输出摘要，搜索jsonl文件可以用subagent来做
记忆不是append就可以，还需要重新调整结构，所以要有编辑能力
等等。其实好像也可以，先这么干，然后在最后重置记忆之前，用 agent 的形式把它重新整理一遍。但是感觉这样好像又有点复杂了。
...
tool result 5
```
然后分别发送1~5号消息给llm api，让其做摘要 & 输出可能的用于grep的关键词（用于联想）
这种好处是省 token 费用，坏处是这些消息之间可能会有联系，你这样切断的话，就失去这个联系了。
我觉得这个方案还是不行，应该要注重效果（提升利润），而不是节省成本，提高利润的上限是很高的，但是成本是有限度的。也不能这么说吧，就是说我认为这个是值得的，就是它这个效果是值得这个成本的。就算现在不值得，那么将来的token费用也是会降低的
