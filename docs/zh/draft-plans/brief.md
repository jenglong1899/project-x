...
随着模型的进步，可能不需要这个了。就是模型记得会在写入一个文件前要先创建这个文件
提醒模型要用EOF格式，这样不用搞转义符。
...
最简单的方法就是在每次操作文件的时候，如果对应的文件夹中没有 git 就新建一个。然后每次编辑工具就会自动 commit 一次，然后 commit message 就是对应编辑的那个 token id。

如果要做 undo 功能的话，那编辑工作必须是自带的工具，而不是通过bash来编辑?
