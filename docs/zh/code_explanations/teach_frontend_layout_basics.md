...
它看起来很奇怪：最小高度为 0，有什么意义？

在 flex 布局里，它经常是为了允许子元素“真的缩小”，从而让内部滚动容器生效。

可以先记一个经验规则：

```text
如果一个 flex 子元素里面还有滚动区域，
并且你希望滚动发生在内部，
那这个 flex 子元素通常需要 min-h-0。
```

否则浏览器可能会按内容的自然高度撑开它，导致滚动不出现在你期望的那个内部容器上。
