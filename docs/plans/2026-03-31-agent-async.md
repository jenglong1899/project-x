...
### 对外接口与约束（保持最小惊讶）
- `Agent.new_conversation()` / `resume_conversation()` / `enqueue_user_message()` / `has_pending_user_messages()` 保持同步。
- 仅把 `Agent.run()` 改为 `async def run(...)`。
- `WebSocketChatSession` 仍然在“生成期间”循环跑 Agent，直到没有 pending user message。
