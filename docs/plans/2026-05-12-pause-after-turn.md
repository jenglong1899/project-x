...
- `AgentRunner` 会在 `Agent.run()` 返回后检查 `drive_decision()`；如果队列里还有消息（用户连发、或前端短时间发了多条），即使本轮没 tool_calls，runner 也会立刻进入下一轮模型调用。
...
- 增加 `submit_pause_request()` / `submit_resume()`，转发到 `AgentRunner`。
...
说明：解除暂停的触发点放在 runner 而非纯前端，确保“发消息自动解除”在所有适配层都一致。
...
- `backend/src/core/agent.py`
  - `enqueue_user_message()`：如果 `paused==True`，自动 `resume()`（双保险；防止未来绕过 runner 直接调用 agent）。
  - `run()` 增加两个“回合边界检查点”：
    1) **无 tool_calls 分支**：assistant message append/persist 后，若 `pause_requested==True`，设置 `paused=True` 并清掉 request，然后 `return` 让 runner idle。
...
- 后端（pytest）：
  ...
  3) paused 后发送消息：自动解除暂停并继续运行（验证 runner 的 submit_user_message 行为）。
