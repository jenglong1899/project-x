...
```text
pointer-events-none
└── 这个元素以及其子元素不要成为鼠标/触摸事件的目标

pointer-events-auto
└── 恢复默认行为，让这个元素可以正常接收鼠标/触摸事件
```
...
```text
浮层外壳
├── pointer-events-none：外壳本身不拦截鼠标
└── 实际提示卡片
    └── pointer-events-auto：卡片重新变得可点击、可 hover
```

为什么要外层 `none`、内层 `auto`？

因为很多浮层会有一个“负责定位的外壳”。这个外壳可能比里面真正显示出来的卡片或按钮更大。如果这个外壳能接收鼠标事件，它就可能挡住下面的聊天区。

这里说的“挡住”，不一定是视觉上挡住。哪怕外层没有背景色、看起来是透明的，只要它参与鼠标命中测试，用户点到它占据的区域时，下面的聊天内容也拿不到这次点击。

所以常见写法是：

```text
外层 absolute 浮层盒子：pointer-events-none
┌──────────────────────────────┐
│  外壳区域：鼠标会穿过去      │
│  ┌────────────────────────┐  │
│  │ 实际提示卡片           │  │
...
└── 普通子元素默认也点不到
    └── 但子元素可以自己写 pointer-events-auto，重新变得可点击
```

也就是说，`pointer-events-auto` 不是“更强的点击效果”，它只是把这个元素恢复成默认的可命中状态。它经常出现在 `pointer-events-none` 的父元素里面，用来把真正需要交互的部分重新打开。

浏览器处理鼠标时，大致会先做一次“命中测试”：

```text
鼠标位置
└── 从最上层元素往下找
    ├── 遇到 pointer-events-none 的元素
    │   └── 跳过这个元素本身
    ├── 如果它的某个子元素显式写了 pointer-events-auto
    │   └── 这个子元素仍然可以成为事件目标
    └── 如果没有命中可接收事件的浮层内容
        └── 继续命中下面的聊天区
```
...
这样按钮周围的外壳区域不会挡住下面的聊天区滚动或选择文本，只有按钮本身能点击。
