...
class ConversationStoreTests(unittest.TestCase):
    def test_start_with_first_user_message_creates_json_with_truncated_display_name(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            store.start_with_first_user_message(user_content="第一条用户消息已经超长")

            stored_files = list(Path(temp_dir).glob("*.json"))
            self.assertEqual(len(stored_files), 1)

            payload = json.loads(stored_files[0].read_text(encoding="utf-8"))
            self.assertEqual(payload["meta"]["display-name"], "第一条用户消息已经超")
            self.assertEqual(
    ...
    def test_truncate_display_name_limits_to_ten_characters(self) -> None:
        self.assertEqual(truncate_display_name("1234567890"), "1234567890")
        self.assertEqual(truncate_display_name("12345678901"), "1234567890")
