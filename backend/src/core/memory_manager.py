...
3. 随着worker做的事情越来越多，应当被记录进记忆文档的东西也会越来越多，你要确保 {MEMORY_MAIN_MD} 只存储80%~90%的情况下都会用到的信息，其他记忆要放到其他文档中，然后在 {MEMORY_MAIN_MD} 里面留下对这些文档的引用就行了。这里的引用不是说所有的其他记忆文档都要被 {MEMORY_MAIN_MD} 直接引用，而是可以被间接引用，比如有20个文档都是关于某个主题的，要把它们都放进一个文件夹里面，然后在 {MEMORY_MAIN_MD} 里面引用这个文件夹就行。
