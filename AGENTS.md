...
### 页面结构
- `frontend/src/App.tsx` 采用左侧信息侧栏、右侧单栏时间线、底部输入区的布局；侧栏里的新会话、刷新列表、工作目录、暂停等功能目前因后端未提供能力而保持禁用态 mock。
...
- `frontend/src/features/chat/store.ts` 的核心状态是 `items[] + pendingUserMessages[] + isGenerating`，不再维护 `assistantTurnId`、active assistant segment map、tool item map 这类中间索引。
...
- `frontend/src/features/chat/client.ts` 已封装 WebSocket 客户端：负责连接 `/ws`、解析 JSON、用 zod 校验服务端事件、把事件喂给 store，并在发送消息时生成 `userMessageId`、先写入 `pendingUserMessages`，再通过 WebSocket 发送命令。客户端会忽略过期 socket 的 `open/message/error/close` 事件，并把主动断开的连接排除出错误路径，以避免 React `StrictMode` 下旧连接污染当前连接状态。
