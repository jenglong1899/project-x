...
#
我不太确定我们现在这种json持久化的方式是不是最优雅的方式。

我发现底层，如果你要修改 json 的话，必须是全量修改，而不能局部修改。然后我看到 conversation store 里面暴露的接口还是说你可以持久化某个数据，而不是全部数据。但它底层本来就是要持久化全部数据的。那我在想是不是干脆你 conversation store 只只对外暴露一个接口叫做 persist 一个叫做 load

我记得 Python 里面是有个库可以直接把一个 Python 对象持久化到一个文件里面去的。
